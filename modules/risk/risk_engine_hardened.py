"""
Hardened Risk Engine with Safe Math Operations
Author: Erdinc Erdogan
Purpose: Production-stable risk engine with numerically-safe VaR, CVaR, EWMA volatility, and drawdown calculations.
References:
- VaR/CVaR (Basel III Compliant)
- Safe Math Operations
- Numerical Stability in Risk Calculations
Usage:
    engine = RiskEngineHardened(returns)
    var = engine.calculate_var(confidence=0.99)
    cvar = engine.calculate_cvar(confidence=0.99)
"""
import numpy as np
from modules.core.safe_math import (
    safe_sqrt, safe_divide, safe_log,
    validate_array, validate_covariance, validate_probability
)
...
class VaRCalculator:
    ...
    def calculate_historical_var(self,
        ...
        var_pct_scaled = var_pct * safe_sqrt(holding_period)
    ...
    def calculate_monte_carlo_var(self,
        ...
        # Scale for holding period
        simulated_returns_scaled = simulated_returns * safe_sqrt(holding_period)
...
class VolatilityForecaster:
    ...
    def calculate_simple_volatility(self,
        ...
        if annualize:
            vol *= safe_sqrt(StatisticalConstants.TRADING_DAYS_YEAR)
        return vol
    ...
    def calculate_ewma_volatility(self,
        ...
        current_vol = safe_sqrt(variance_series[-1])
        
        if annualize:
            current_vol *= safe_sqrt(StatisticalConstants.TRADING_DAYS_YEAR)
            vol_series = safe_sqrt(variance_series) * safe_sqrt(StatisticalConstants.TRADING_DAYS_YEAR)
        else:
            vol_series = safe_sqrt(variance_series)
    ...
    def forecast_volatility(self,
        ...
        # Scale for horizon
        horizon_vol = base_vol * safe_sqrt(horizon_days)
        
        # Annualize
        return horizon_vol * safe_sqrt(StatisticalConstants.TRADING_DAYS_YEAR / horizon_days)
...
class RiskMetricsEngine:
    ...
    def calculate_sortino_ratio(self,
        ...
        # Downside deviation
        downside_dev = safe_sqrt(np.mean((downside_returns - target) ** 2))
        downside_dev_annual = downside_dev * safe_sqrt(StatisticalConstants.TRADING_DAYS_YEAR)
    ...
    def calculate_information_ratio(self,
        ...
        tracking_error = np.std(excess_returns, ddof=1) * safe_sqrt(StatisticalConstants.TRADING_DAYS_YEAR)
