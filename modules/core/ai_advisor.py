"""
AI Advisor Full Edition - Hedge Fund Level Multi-Modal AI
Author: Erdinc Erdogan
Purpose: Comprehensive hedge fund AI advisor with multi-modal vision analysis, property
graphs, dynamic model orchestration, and async pipelines for institutional trading.
References:
- Multi-Modal Vision LLM for Chart Analysis
- Property Graph Knowledge Extraction
- Corrective RAG (C-RAG) with Dynamic Thresholds
Usage:
    advisor = AIAdvisor(mode="full")
    result = await advisor.analyze(ticker="AAPL", include_vision=True)
"""
...
try:
    ...
    print("UYARI: CrossEncoder bulunamadı.")

# Cross-Encoder için - TensorFlow tamamen suppress edilmiş
# yfinance
try:
...
AL/SAT/BEKLE öner:"""
    ...
    def analyze_trade(
        ...
        # 10. Graph-RAG (Ekonomik İlişkiler)
        ...
        # 10.5 Property Graph (Ripple Effects) - YENİ
        ripple_effects = ""
        try:
            ripple_effects = self.property_graph.get_ripple_effects(ticker)
            if ripple_effects:
                print(f"{Fore.MAGENTA}  → Property Graph: Sektör ilişkileri analiz edildi{Style.RESET_ALL}", flush=True)
                context = ripple_effects + "\n" + context
        except:
            pass
        
        # 10.6 Vision-RAG (Grafik Analizi) - YENİ (Vision model varsa)
        vision_analysis = ""
        if self.vision.available:
            try:
                # Fiyat verisinden grafik oluştur ve analiz et
                chart_path = self.vision.capture_chart_screenshot(ticker)
                if chart_path:
                    vision_analysis = self.vision.analyze_chart(chart_path, f"{ticker} için OBO, Double Top veya destek/direnç kırılımı var mı?")
                    if vision_analysis:
                        print(f"{Fore.GREEN}  → Vision-RAG: Grafik formasyonları analiz edildi{Style.RESET_ALL}", flush=True)
                        context = vision_analysis + "\n" + context
            except Exception as e:
                print(f"{Fore.YELLOW}  → Vision-RAG: {e}{Style.RESET_ALL}", flush=True)
