"""
Hardened GARCH Volatility Forecasting with Safe Math
Author: Erdinc Erdogan
Purpose: Production-stable GARCH implementation with numerically-safe operations for MLE estimation and volatility forecasting.
References:
- GARCH (Bollerslev, 1986)
- Numerical Stability in Optimization
- Safe Math Operations
Usage:
    model = GARCHModelHardened(model_type=GARCHType.GARCH_11)
    model.fit(returns)
    forecast = model.forecast(horizon=10)
"""
import numpy as np
from modules.core.safe_math import (
    safe_log, safe_sqrt, safe_divide, safe_exp,
...
class GARCHParams:
    ...
    def long_run_volatility(self) -> float:
        ...
        return safe_sqrt(self.long_run_variance * 252)
    ...
    def half_life(self) -> float:
        ...
        if self.persistence <= 0 or self.persistence >= 1:
            return np.inf
        return safe_log(0.5) / safe_log(self.persistence)
...
class VolatilityForecast:
    ...
    def get_forecast(self, horizon: int) -> float:
        ...
        return self.long_run_volatility / safe_sqrt(252)
...
class GARCH11:
    ...
    def fit(self, returns: np.ndarray, 
        ...
        self.residuals = self.returns / safe_sqrt(self.conditional_variance)
        self._fitted = True
    ...
    def _negative_log_likelihood(self, params: np.ndarray) -> float:
        ...
        if self.distribution == DistributionType.STUDENT_T:
            ll = np.sum(student_t.logpdf(
                self.returns / safe_sqrt(sigma2), 
                df=garch_params.nu,
                scale=safe_sqrt(sigma2)
            ))
        else:
            ll = np.sum(norm.logpdf(self.returns, scale=safe_sqrt(sigma2)))
    ...
    def forecast(self, horizon: int = 20) -> VolatilityForecast:
        ...
        for h in range(horizon):
            ...
            term_structure[h] = safe_sqrt(sigma2_h)
        
        # Volatility of volatility (approximate)
        vol_of_vol = np.std(safe_sqrt(self.conditional_variance[-60:])) * safe_sqrt(252)
        
        return VolatilityForecast(
            current_volatility=safe_sqrt(sigma2_t),
            forecast_1d=term_structure[0] if horizon >= 1 else safe_sqrt(sigma2_t),
            ...
            annualized_volatility=safe_sqrt(sigma2_t * 252),
    ...
    def get_dynamic_cvar(self, confidence: float = 0.95) -> float:
        ...
        sigma_t = safe_sqrt(self.conditional_variance[-1])
...
║  📈 VOLATILITY METRICS                                                       ║
║  ────────────────────────────────────────────────────────────────────────    ║
║  Current Vol (daily):    {safe_sqrt(self.conditional_variance[-1])*100:.2f}%                                  ║
║  Current Vol (annual):   {safe_sqrt(self.conditional_variance[-1]*252)*100:.1f}%                                  ║
...
class EGARCH:
    ...
    def fit(self, returns: np.ndarray) -> Dict:
        ...
        self.params = {
            "omega": safe_log(sample_var) * 0.05,
            "alpha": 0.10,
            "beta": 0.85,
