...
Purpose: Estimates slippage using Almgren-Chriss framework with temporary/permanent impact decomposition and optimal execution trajectory calculation.
References:
- Almgren & Chriss (2000): Optimal Execution of Portfolio Transactions
- Temporary vs Permanent Impact Decomposition
- Risk-Adjusted Cost Minimization
Usage:
    model = AlmgrenChrissModel(params=AlmgrenChrissParams())
...
class TemporaryImpactModel:
    ...
    def calculate(self, 
        ...
        impact = self.eta * volatility * np.power(participation, self.beta)
...
class AlmgrenChrissModel:
    ...
    def estimate_slippage(self,
        ...
        slippage_std = volatility * np.sqrt(participation) * 0.5
    ...
    def _optimal_execution_time(self,
        ...
        if participation <= max_participation:
            ...
            time_factor = np.sqrt(participation / 0.01)  # Scale with size
    ...
    def optimal_trajectory(self,
        ...
        sigma = volatility / np.sqrt(390)  # Per-minute volatility
        
        # Almgren-Chriss kappa parameter
        # κ = sqrt(λ * σ² / η)
        kappa = np.sqrt(lambda_risk * sigma**2 / (eta + 1e-10))
